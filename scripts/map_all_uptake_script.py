...
if __name__ == "__main__":
    ...
    scout_data.filter_records("C_name", ["Northamptonshire"])
    ...
    pcon.filter_boundaries(field="C_name", value_list=["Northamptonshire"], boundary="pcon", distance=3000, near=True)
    pcon.create_boundary_report(["Section numbers", "6 to 17 numbers"], historical=False, report_name="pcon_Northamptonshire")
    pcon.create_uptake_report(report_name="pcon_Northamptonshire_uptake_report")

    imd = Reports("lsoa", scout_data)  # As is expens
    ...
    map = Map(scout_data, map_name="Northamptonshire_uptake_map4")
    ...
    map.set_region_of_colour("C_name", ["Northamptonshire"])
    ...
    map.add_meeting_places_to_map(scout_data.data.loc[scout_data.data["C_name"] == "Northamptonshire"], map.district_colour_mapping(), ["youth membership"], "Your Sections")
    map.save_map()
