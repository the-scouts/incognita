...
if __name__ == "__main__":
    ...
    pcon.filter_boundaries_near_scout_area("pcon" , "X_name", ["Wales"], exec_tm=True)
    pcon.create_boundary_report(["Section numbers", "6 to 17 numbers"], historical=False, report_name="pcon_wales", exec_tm=True)
    pcon.create_uptake_report(report_name="pcon_wales_uptake_report", exec_tm=True)
    ...
    imd.create_boundary_report(["Section numbers", "6 to 17 numbers"], historical=False, report_name="imd_wales", exec_tm=True)
    ...
    lad = Boundary("lad", scout_data)
    lad.filter_boundaries("pcon", pcon_list)
    lad.create_boundary_report(["Section numbers", "6 to 17 numbers"],
                               report_name="las_wales")
    lad.create_uptake_report(report_name="las_wales_uptake_report", exec_tm=True)

    max_year = scout_data.max_year

    # create_6_to_17_map
    dimension = {"column": f"%-All-{max_year}", "tooltip": "% 6-17 Uptake", "legend": "% 6-17 Uptake (pcon)"}
    map = Map(scout_data, map_name="pcon_imd_wales_uptake_map")
    ...
    dimension = {"column": "All-2019", "tooltip": "Ages 6-17", "legend": "Districts"}
    ...
    lad.filter_records_by_boundary(exec_tm=True)
